...
Para comprobar que la operación se realizó con éxito debemos revisar el ambiente de trabajo (*environment*). En dicha ventana, se nos indica la cantidad de observaciones (`r  nrow(apellidos)`) y variables (`r ncol(apellidos)`) que tiene el objeto en cuestión. Si hacemos *click* en el mismo, accederemos a la base de datos completa.  
...
Otra de las potencialidades que ofrecen estas funciones es que permiten leer datos directamente desde alguna URL de la web. Por ejemplo, hagamos una prueba importando los *datasets* que hemos seleccionado para el curso y se encuentran disponibles en el [repositorio de datos abiertos de Argentina](https://datos.gob.ar/){target="_blank"}.

En primer lugar, seleccionaremos la base de datos de turismo receptivo. Para ello, utilizaremos la función `read_csv()` del paquete `readr` y le pasaremos como argumento la URL del archivo.

```{r}
turismo_receptivo <- read_csv("https://datos.yvera.gob.ar/dataset/4cbf7d4a-702a-4911-8c1e-717a45214902/resource/fdfe0ae4-4acc-4421-aa48-6149a02bc615/download/turistas-no-residentes-serie.csv")

```

```{r echo=FALSE}
DT::datatable(turismo_receptivo)
```

Realizaremos la importación del resto de los datasets que usaremos en el curso.  
...
Al observar la clase del objeto `consumos_culturales`, podemos ver que es un *data.frame*, específicamente en formato *tibble*. Este es el tipo de objeto que se utiliza en `R` para almacenar bases de datos de estructura tabular.

```{r}
class(consumos_culturales)
```
...
Por ejemplo, en la base de graduados contamos con la variable `rama_id`, que indica la rama del conocimiento en la que se graduó la persona. Si bien la variable fue cargada como numérica, sabemos que, en realidad, es una variable categórica. Para convertirla en factor, usamos la función `factor()`, en donde, además, podemos declarar las distintas etiquetas (*labels*) que tienen las categorías. Para no sobreescribir la variable original, la guardamos en una nueva variable llamada `rama_factor`.

```{r}
graduadxs_universit$rama_factor <- factor(graduadxs_universit$rama_id, 
                                    labels = c(
                                      "Ciencias Sociales",
                                      "Ciencias Aplicadas",
...
Con la función `levels()` podemos ver los niveles que tiene la variable `rama_factor``, es decir, como están ordenadas las categorías.

```{r}
levels(graduadxs_universit$rama_factor)
```
